...
public class WDFragment extends Fragment {
    ...
    public WDFragment(Z_SHZSActivity activity) {
    ...
    private void setData() {
        ...
        for (int i = 0; i < shzsList.size(); i++) {
            ...
            barEntries.add(new Entry(i, shzs.getTemperature()));
            integers.add(shzs.getTemperature());
            xValue.add(((i + 1) * 3) + "");
        }
        ...
        tvTitle.setText("过去1分钟最高气温：" + integers.get(integers.size() - 1) + "℃,最低气温" + integers.get(0) + "℃");
