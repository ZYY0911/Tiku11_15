...
public class COFragment extends Fragment {
    ...
    public COFragment(Z_SHZSActivity activity) {
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        barChart.setBackgroundColor(Color.parseColor("#52C6F7"));
    ...
    private void setData() {
        ...
        for (int i = 0; i < shzsList.size(); i++) {
            ...
            barEntries.add(new Entry(i,shzs.getCo2()));
            integers.add(shzs.getCo2());
            xValue.add(((i+1)*3)+"");
        }
        ...
        tvTitle.setText("过去1分钟最大相对浓度："+integers.get(integers.size()-1));
        LineData data = new LineData(dataSet);
        barChart.getDescription().setText("(S)");
        barChart.getLegend().setEnabled(false);
        YAxis yAxis = barChart.getAxisLeft();
        yAxis.setDrawAxisLine(false);
        yAxis.setAxisMaximum(8200);
